...
const sequence = [
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Function to update the shortcut list dynamically
    function changeBG(text) {
        const shortcutsList = document.getElementById(text);
        shortcutsList.style.backgroundColor = '#ddc23c';

        setTimeout(() => {
            shortcutsList.style.backgroundColor = 'white'; // Reset to default
          }, 100);
    };
  
    // Play the beat and update shortcuts
    const playBeat = () => {
      const audio = new Audio(`sounds/PH_Intro.mp3`);
      audio.play();
      sequence.forEach(([sound, text, time], index) => {
        setTimeout(() => {
          changeBG(text);
        }, time);
      });
